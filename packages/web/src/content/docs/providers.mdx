...
OpenCode has **built-in support for LM Studio** — it automatically discovers the models you have installed by querying LM Studio's local API. No need to list models manually.

1. Start LM Studio and make sure the **Local Server** is running (default port: `1234`).

2. Run the `/models` command in OpenCode — your installed models will appear automatically under **lmstudio**.

   ```txt
   /models
   ```

That's it. OpenCode detects which models are loaded in LM Studio and offers only those, preventing "invalid model identifier" errors.

:::tip
If LM Studio is unreachable (not running or wrong port), the provider is silently skipped. Start LM Studio's local server and restart OpenCode to pick it up.
:::

#### Custom base URL

By default OpenCode connects to `http://127.0.0.1:1234/v1`. Override this if you run LM Studio on a different host or port:
...
LM Studio doesn't require an API key by default. If you've configured one, pass it via `/connect` or the `LMSTUDIO_API_KEY` environment variable:

```bash
LMSTUDIO_API_KEY=my-key opencode
```
